...
train_low_data = []
train_high_data = []
train_low_data_names = glob('./LOLdataset/all-plants/val/600x400/low/*')
train_low_data_names.sort()
train_high_data_names = glob('./LOLdataset/all-plants/val/600x400/high/*')
...
val_low_data = []
val_high_data = []
val_low_data_names = glob('./LOLdataset/all-plants/val/600x400/low/*')
val_low_data_names.sort()
print('[*] Number of val data: %d' % len(val_low_data_names))
for idx in range(len(val_low_data_names)):
    val_low_im = load_images(val_low_data_names[idx])
    val_low_data.append(val_low_im)
    val_high_im = load_images(val_high_data_names[idx])
    val_high_data.append(val_high_im)
...
val_decomposed_low_i_data = []
val_decomposed_high_i_data = []
for idx in range(len(val_low_data)):
    val_input_low = np.expand_dims(val_low_data[idx], axis=0)
    val_RR, val_II = sess.run([decom_output_R, decom_output_I], feed_dict={input_decom: val_input_low})
    val_RR0 = np.squeeze(val_RR)
    val_II0 = np.squeeze(val_II)
    print(val_RR0.shape, val_II0.shape)
    val_decomposed_low_i_data.append(val_II0)
eval_adjust_low_i_data = val_decomposed_low_i_data
train_adjust_low_i_data = decomposed_low_i_data_480
train_adjust_high_i_data = decomposed_high_i_data_480
