...
train_low_data1 = []
train_high_data1 = []
train_low_data_names = glob('./LOLdataset/NewLOL/train/LOL/low/*.png')
train_low_data_names.sort()
train_high_data_names = glob('./LOLdataset/NewLOL/train/LOL/high/*.png')
train_high_data_names.sort()
assert len(train_low_data_names) == len(train_high_data_names)
for idx in range(len(train_low_data_names)):
    low_im = load_images(train_low_data_names[idx])
    train_low_data1.append(low_im)
    high_im = load_images(train_high_data_names[idx])
    train_high_data1.append(high_im)
train_low_data = []
train_high_data = []
train_low_data_names = glob('./LOLdataset/all-plants/train/600x400/low/*')
train_low_data_names.sort()
train_high_data_names = glob('./LOLdataset/all-plants/train/600x400/high/*')
train_high_data_names.sort()
assert len(train_low_data_names) == len(train_high_data_names)
...
eval_low_data = []
eval_high_data = []
eval_low_data_name = glob('./LOLdataset/all-plants/val/600x400/low/*')
eval_low_data_name.sort()
eval_high_data_name = glob('./LOLdataset/all-plants/val/600x400/high/*')
eval_high_data_name.sort()
assert len(eval_low_data_name) == len(eval_high_data_name)
print('[*] Number of training data: %d' % len(eval_low_data_name))
...
for epoch in range(start_epoch, epoch):
    ...
    for batch_id in range(start_step, numBatch):
        ...
        iter_num += 1
    fig_loss[epoch] = np.sum(epoch_train_loss) / numBatch
